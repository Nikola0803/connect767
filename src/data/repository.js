...
export async function submitListing(form) {
  ...
  const created = await customClient.post("/listings", payload);
  await setBookingEnabled(created?.id, form.bookingEnabled);
  return created;
}

/**
 * Bookings live behind their own endpoint (class-rest-listings.php's
 * /listings/{id}/booking), not the listing create/update payload — so it has
 * to be a second call. Appending `bookingEnabled` to the listing FormData
 * looks like it works and silently does nothing, because update() never reads
 * that key.
 *
 * Deliberately non-fatal: a listing that saved but whose booking flag didn't
 * shouldn't report the whole submission as failed. The toggle is one click to
 * retry; a lost listing is not.
 */
/**
 * Public booking request against a listing.
 *
 * Backed by class-rest-bookings.php, which has existed all along — the
 * frontend simply never had a form, so turning bookings on did nothing
 * visible on the profile. The endpoint re-checks `booking_enabled` server
 * side and rejects the request if the owner has since switched it off.
 */
export async function submitBooking(slug, payload) {
  if (!isLiveApi) {
    await new Promise((resolve) => setTimeout(resolve, 500));
    return { id: "local-preview", status: "pending" };
  }
  return customClient.post(`/listings/${slug}/bookings`, payload);
}

/**
 * Change an existing listing's plan from the account dashboard.
 *
 * Returns `{ paymentRequired }` — moving to Classified marks the listing
 * unpaid and the caller is expected to send the owner through the same
 * payment flow the submission wizard uses. Nothing is granted for free.
 */
export async function setListingTier(listingId, tier) {
  if (!isLiveApi) {
    await new Promise((resolve) => setTimeout(resolve, 300));
    return { id: listingId, tier, paymentRequired: tier === "classified" };
  }
  return customClient.post(`/listings/${listingId}/tier`, { tier });
}

async function setBookingEnabled(listingId, enabled) {
  if (!listingId) return;
  try {
    await customClient.post(`/listings/${listingId}/booking`, { enabled: enabled ? 1 : 0 });
  } catch {
    /* listing itself saved fine — surfacing this would be more alarming than useful */
  }
}
...
export async function updateListing(id, form) {
  ...
  const updated = await customClient.post(`/listings/${id}`, payload);
  await setBookingEnabled(id, form.bookingEnabled);
  return updated;
}
