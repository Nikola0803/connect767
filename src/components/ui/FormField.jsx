...
/**
 * Multi-line input, vertically resizable by default.
 *
 * These were locked to `resize-none`, which meant a long description had to be
 * written through a four-line slot — the grab handle browsers give you for
 * free was being switched off. Vertical only: horizontal dragging breaks the
 * surrounding grid layouts.
 *
 * Pass `resizable={false}` where a fixed height genuinely matters.
 */
export function Textarea({ className = "", error, resizable = true, ...props }) {
  return (
    <textarea
      className={`${baseFieldClass} ${resizable ? "resize-y min-h-[80px]" : "resize-none"} ${
