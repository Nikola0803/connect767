...
function GltfGarment({ url, color, colorB, splitAt = 0.5, softness = 0.5 }) {
  ...
  useMemo(() => {
    // World-space Y bounds, so the split/gradient can be expressed as "waist
    // height" rather than a magic number that only suits one model.
    ...
    cloned.traverse((child) => {
      if (child.isMesh && child.material && !child.name.includes("no-recolor")) {
        ...
        const mat = child.userData.c767BaseMaterial.clone();
        mat.color.set(color);

        /**
         * Vertical two-colour blend, injected into the standard material.
         *
         * These scans are a SINGLE mesh with one material — there is no
         * separate "shirt" and "shorts" submesh to colour independently. So
         * the split is done in the shader by height: everything above the
         * waistline takes the top colour, everything below the bottom colour,
         * with `softness` controlling whether that's a hard seam (a shirt/
         * shorts split) or a smooth fade (a gradient kit).
         *
         * One mechanism covers both features the client asked for, and it
         * needs no changes to the model.
         */
        if (colorB) {
          mat.onBeforeCompile = (shader) => {
            shader.uniforms.c767ColorA = { value: new THREE.Color(color) };
            shader.uniforms.c767ColorB = { value: new THREE.Color(colorB) };
            // splitAt is 0..1 up the garment; 0.5 is roughly the waist.
            shader.uniforms.c767Split = { value: minY + (maxY - minY) * splitAt };
            shader.uniforms.c767Soft = { value: Math.max(0.0001, softness * (maxY - minY)) };

            shader.vertexShader = shader.vertexShader
              .replace(
                "#include <common>",
                "#include <common>\nvarying float vC767Y;",
              )
              .replace(
                "#include <begin_vertex>",
                "#include <begin_vertex>\nvC767Y = (modelMatrix * vec4(transformed, 1.0)).y;",
              );

            shader.fragmentShader = shader.fragmentShader
              .replace(
                "#include <common>",
                "#include <common>\nvarying float vC767Y;\nuniform vec3 c767ColorA;\nuniform vec3 c767ColorB;\nuniform float c767Split;\nuniform float c767Soft;",
              )
              .replace(
                "#include <color_fragment>",
                "#include <color_fragment>\n  float c767T = smoothstep(c767Split - c767Soft, c767Split + c767Soft, vC767Y);\n  diffuseColor.rgb *= mix(c767ColorB, c767ColorA, c767T) / max(vec3(0.0001), diffuse);",
              );
          };
          // Materials with different shader programs must not share a cache
          // key, or three reuses the first compiled program for all of them.
          mat.customProgramCacheKey = () => `c767-${color}-${colorB}-${splitAt}-${softness}`;
        }
  ...
  }, [cloned, color, colorB, splitAt, softness]);
...
/**
 * @param colorB   Second colour. Omit for a plain single-colour garment.
 * @param splitAt  Height of the boundary, 0 (hem) to 1 (collar). ~0.5 = waist.
 * @param softness 0 = hard seam (shirt vs shorts), 1 = full-length fade.
 */
export default function GarmentModel({ productType, color, colorB, splitAt, softness }) {
  ...
  if (url) {
    return (
      <Suspense fallback={<FallbackPrimitive slug={slug} color={color} />}>
        <GltfGarment url={url} color={color} colorB={colorB} splitAt={splitAt} softness={softness} />
      </Suspense>
    );
  }
