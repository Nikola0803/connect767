...
export default function Hero() {
  ...
  // Same 7 top-level categories + live listing counts (parent counts already
  // roll up their nested industries) as the "Browse by category" section
  // and the searchbar's own Industry dropdown — see Categories.jsx /
  // getHomeCategories() in data/repository.js.
  const { data: trendingCategories } = useAsync(() => getHomeCategories(), []);
      ...
      <div className="relative z-10 w-full px-4 md:px-8 lg:px-12 pb-16 md:pb-24 pt-32 md:pt-40">
        <div className="max-w-5xl">
          ...
          <p className="mt-6 max-w-2xl text-background-50/85 text-base md:text-lg font-light">
            ...
            the people who make your town feel like home.
          </p>
          ...
          <div className="mt-5 flex flex-wrap items-center gap-2">
            {(trendingCategories || []).map((cat) => (
              <Link
                ...
                className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-background-50/10 hover:bg-background-50/20 border border-background-50/20 text-background-50 text-xs font-medium cursor-pointer whitespace-nowrap transition-colors"
              >
                {cat.name}
                <span className="text-background-50/60">
                  {(cat.count || "").replace(" listings", "")}
                </span>
              </Link>
            ))}
          </div>
