...
/**
 * Keyless map embed for a free-text location.
 *
 * VITE_MAPBOX_TOKEN lives in .env / .env.local, both of which are gitignored
 * — and the VPS builds from a fresh `git clone`, so production has no token
 * and every Mapbox call silently no-ops. That's why maps vanished on the live
 * site while working locally. (config.js already hit this exact trap for
 * VITE_WP_BASE_URL and papers over it with a hardcoded production fallback;
 * a Mapbox token can't be hardcoded the same way.)
 *
 * Google's `output=embed` needs no API key, so this always renders something
 * real. Mapbox is still preferred when a token IS configured — it looks
 * better and doesn't frame-load a third party — this is the floor, not the
 * ceiling.
 */
export function embedMapUrl(query) {
  if (!query) return "";
