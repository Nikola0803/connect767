...
class C767_Meta_Fields
{
    ...
    public static function listing_fields()
    {
        return [
            ...
            // Rendered as a <select>. It was a free-text box among ~40 other
            // fields: hard to spot, and it silently required the exact string
            // "Classified" — typing "classified" looked right and matched
            // nothing, so the listing stayed Free with no error shown.
            'tier' => [
                'label' => 'Listing tier',
                'type' => 'string',
                'options' => ['Free', 'Featured', 'Classified'],
            ],
            // Needed alongside `tier` when an admin grants Classified by hand
            // (a comp, or a payment taken offline). Without it the listing
            // shows as Classified on the site while the moderation screen
            // still reports it unpaid, which reads as a billing problem that
            // isn't one. Set this to "paid" whenever you set the tier
            // manually; the Stripe/PayPal callbacks set it themselves.
            'payment_status' => [
                'label' => 'Payment status (for manually-set Classified)',
                'type' => 'string',
                'options' => ['n/a', 'unpaid', 'paid'],
            ],
            ...
            'degree_type' => ['label' => 'Degree type / field of study (e.g. Nursing, Business Administration)', 'type' => 'string'],
            'experience_level' => [
                'label' => 'Experience level',
                'type' => 'string',
                'options' => ['', '0-5 years', '5-10 years', '10-15 years', '15-20 years', '20+ years'],
            ],
            'years_experience' => ['label' => 'Years of experience (exact number, shown on the profile)', 'type' => 'string'],
            ...
            'logo_position' => ['label' => 'Logo crop position (e.g. "50% 50%", set by dragging in the listing form)', 'type' => 'string'],
            'logo_zoom' => ['label' => 'Logo zoom (1.0-2.5)', 'type' => 'string'],
            'cover_position' => ['label' => 'Cover photo crop position (e.g. "50% 50%", set by dragging in the listing form)', 'type' => 'string'],
    ...
    private static function render_fields_table($post, $fields)
    {
        ...
        foreach ($fields as $key => $def) {
            ...
            } elseif (!empty($def['options'])) {
                printf('<select id="c767_%1$s" name="c767_%1$s">', esc_attr($key));
